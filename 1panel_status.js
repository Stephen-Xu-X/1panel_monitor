...
// icon-color: green; icon-glyph: battery-full;

/**
 * 1Panel Monitor (Perfect Native v10)
 * 修复: 所有组件文字颜色均自动适配深浅模式 (浅色黑字/深色白字)
 * 保持: 颜色渐变、布局、ISP显示等所有特性
 */
...
const THEME = {
  ...
  // 深色背景: Secondary System Background
  ...
  // 渐变色定义
  colorStart: new Color("#30d158"), // iOS Green
  colorEnd: new Color("#FF453A"),   // iOS Red
...
try {
  ...
  if (config.runsInWidget) {
    ...
    // 👇 预览修改这里: presentSmall(), presentMedium(), presentLarge()
    widget.presentLarge(); 
  }
  Script.complete();
} catch (e) {
...
async function getData() {
  ...
  let cachePath = fm.joinPath(fm.documentsDirectory(), "1panel_ui_v10.json");
...
function renderSmall(w, data) {
  ...
  let img = drawCircleWithText(data.cpu, "CPU", size, 10);
  box.addImage(img);
...
function getGradientColor(val) {
  let pct = Math.max(0, Math.min(1, val / 100)); // 0~1
...
function drawCircleWithText(item, centerLabel, size, strokeWidth) {
  ...
  if (pct > 0) {
    ...
    if (pct < 1) {
      fillCircle(ctx, center, radius, startAngle + sweepAngle, strokeWidth/2, dynamicColor);
    }
  }
...
function addBar(w, label, item) {
  ...
  if (item.val > 0) {
    ...
    draw.addPath(p2); 
    draw.setFillColor(dynamicColor);
    draw.fillPath();
  }
